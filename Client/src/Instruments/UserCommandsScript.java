...
public class UserCommandsScript extends UserCommands {
    ...
    public static void execute(String userCommand, BufferedReader reader, HashSet<Flat> flats, LocalDateTime today, ArrayList<String> history, ArrayList<String> scriptCommands, int i, DBHandler db, Integer userId) throws IOException {      // Выполняет команду, если status == 1
        if (UserCommands.status == 1){
            ...
            if (userCommand.equalsIgnoreCase("add")) {
                ScriptAdd add = new ScriptAdd(flats, scriptCommands, i, db);
            ...
            if (userCommand.equalsIgnoreCase("clear")) {
                Clear clear = new Clear(flats, db);
            ...
            try {
                if (userCommand.substring(0, 6).equalsIgnoreCase("update")) {
                    ...
                    if (control != 0) {
                        ScriptUpdate update = new ScriptUpdate(flats, id, scriptCommands, i, db);
            ...
            try {
                if (userCommand.substring(0, 12).equalsIgnoreCase("remove_by_id")) {
                    ...
                    if (control != 0) {
                        Remove_By_Id rmi = new Remove_By_Id(flats, id, db);
            ...
            try {
                if (userCommand.substring(0, 14).equalsIgnoreCase("remove_greater")) {
                    ...
                    Remove_Greater rg = new Remove_Greater(flats, numberOfRooms, db);
                    rg.setUserId(userId);
                    rg.execute();
            ...
            try {
                if (userCommand.substring(0, 12).equalsIgnoreCase("remove_lower")) {
                    String[] parts = userCommand.split(" ", 2);
                    int numberOfRooms = Integer.parseInt(parts[1]);
                    Remove_Lower rl = new Remove_Lower(flats, numberOfRooms, db);
                    rl.setUserId(userId);
                    rl.execute();
